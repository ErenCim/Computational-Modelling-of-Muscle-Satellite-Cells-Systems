...
CSV_PATH        = Path("/Users/WangGroup_UofT/Desktop/Coding Projects/Thesis/initial_conditions.csv")
...
def maybe_plot_dense(y0_vec: np.ndarray, theta_log: torch.Tensor, sample_id: int, num_pts: int = 400) -> None:
    ...
    if PLOT_EVERY <= 0 or ((sample_id % PLOT_EVERY) != 0 and sample_id not in PLOT_VALUES) or sample_id in DONT_PLOT:
        return
    ...
    for i, lab in enumerate[str](STATE_ORDER):
        ...
        plt.plot(t_plot, Y[i], label="Figure From Fitted Params", color='blue')

        filtered_data = training_samples[training_samples['sample_id'] == sample_id]
        x = filtered_data['t'].values
        y = filtered_data[lab].values

        plt.scatter(x, y, color='red', s=30, label="Figure From Collected Data")
        for xi, yi in zip(x, y):
            # format(yi, '.2f') keeps it to 2 decimal places
            plt.annotate(f'{yi:.2f}', 
                        (xi, yi),                   # The point to label
                        textcoords="offset points", # How to position the text
                        xytext=(0, 3),             # Distance from the point (x, y)
